...
function ContainsDX_FourOP_Data(dmp: TMemoryStream; var StartPos: integer;
  const Report: TStrings): boolean;
function ContainsDX_FourOP_VCED(dmp: TMemoryStream;
  var StartPos, StartDmp: integer): boolean;
function ContainsDX_FourOP_ACED(dmp: TMemoryStream;
  var StartPos, StartDmp: integer): boolean;
function ContainsDX_FourOP_ACED2(dmp: TMemoryStream;
  var StartPos, StartDmp: integer): boolean;
function ContainsDX_FourOP_ACED3(dmp: TMemoryStream;
  var StartPos, StartDmp: integer): boolean;
function ContainsDX_FourOP_VMEM(dmp: TMemoryStream;
  var StartPos, StartDmp: integer): boolean;
        ...
        begin
          ...
          begin
            ...
            case rHeader.f of    //0, 1, 2, 5, 6, 9
              $00: begin
                if dmp.Size - StartPos < 163 then
                begin
                  Result := False;
                  tmpList.Add('Data size less than 163 bytes');
                end
                else
                  Result := True;
              end;
              ...
              $01: begin
                if dmp.Size - StartPos < 102 then
                begin
                  Result := False;
                  tmpList.Add('Data size less than 102 bytes');
                end
                else
                  Result := True;
              end;
              ...
              $02: begin
                if dmp.Size - StartPos < 1128 then
                begin
                  Result := False;
                  tmpList.Add('Data size less than 1128 bytes');
                end
                else
                  Result := True;
              end;
              ...
              $05: begin
                if dmp.Size - StartPos < 82 then
                begin
                  Result := False;
                  tmpList.Add('Data size less than 82 bytes');
                end
                else
                  Result := True;
              end;
...
var
  strStream: TStringStream;
  rHeader: TDXSysExHeader;
  fValues: array [0..2] of byte = ($03, $04, $7E);
  tmpPosition: int64;
  tmpList: TStringList;
begin
  Result := False;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if PosEx('LM  8054DL', strStream.DataString, 1) > 0 then
                tmpList.Add('DS55 Delay - DELAY');
              if PosEx('LM  8036EF', strStream.DataString, 1) > 0 then
                tmpList.Add('YS Effects - EFEDS');
              strStream.Free;
              dmp.Position := tmpPosition;
            end;
          end;
...
var
  dummy: byte;
  LM: array[0..9] of char;
begin
  if StartPos <= dmp.Size then
  begin
    dmp.Position := StartPos;
    StartPos := -1;
    while (StartPos = -1) and (dmp.Position < dmp.Size - 16) do
    begin
      dummy := dmp.ReadByte;
      if dummy = $F0 then                             // $F0 - SysEx
      begin
        ...
        dummy := dmp.ReadByte;                        // sub-status + channel number
        dummy := dmp.ReadByte;
        if not (dummy = $7E) then StartPos := -1;     // $7E - Universal dump
        dummy := dmp.ReadByte;
        if not (dummy = $00) then StartPos := -1;     // byte count MS
        dummy := dmp.ReadByte;
        if not (dummy = $21) then StartPos := -1;     // byte count LS
        dmp.Read(LM, 10);
        if not (LM = 'LM  8976AE') then StartPos := -1;
      end;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if not (dummy = $00) then StartPos := -1;     // byte count MS
        dummy := dmp.ReadByte;
        if not (dummy = $14) then StartPos := -1;     // byte count LS
        dmp.Read(LM, 10);
        if not (LM = 'LM  8023AE') then StartPos := -1;
      end;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if not (dummy = $00) then StartPos := -1;     // byte count MS
        dummy := dmp.ReadByte;
        if not (dummy = $1e) then StartPos := -1;     // byte count LS
        dmp.Read(LM, 10);
        if not (LM = 'LM  8073AE') then StartPos := -1;
      end;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        dummy := dmp.ReadByte;                        // sub-status + channel number
        dummy := dmp.ReadByte;
        if not (dummy = $04) then StartPos := -1;     // $03 - 1 Voice dump
        dummy := dmp.ReadByte;
        if not (dummy = $20) then StartPos := -1;     // byte count MS
        dummy := dmp.ReadByte;
        if not (dummy = $00) then StartPos := -1;     // byte count LS
